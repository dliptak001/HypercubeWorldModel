...
- Hot-path buffers: Head keeps one packed-cube scratch; PlanCost walks the rollout in place (O(code) extra, not O(B·H·code)). VectorModel::Rollout encodes into its action_codes_ member. Python fit gathers each batch into a reused workspace. Inference methods take optional out= (C-contiguous writeable float32 of the result shape; same object returned).
- Memory/lifetime: Encoder snapshots the episode field so Encode(LastRawCube()) is safe; lin_r2 and rollout_error_from_codes reject mismatched shapes instead of overreading; VectorModel Save/Load stream the HWM1 payload (no extra in-memory copy). Python last_cube after a batched encode is documented as the last row only; Head and batched methods document GIL release / not thread-safe.
