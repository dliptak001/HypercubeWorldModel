...
## Unreleased
- Python SDK: restore_best defaults on for WorldModel and Decoder; WorldModel.passes and config()["passes"] are as given (0 stays 0); Head.predict (and Head(z)) is the scalar map; Head.plan_cost(zs) takes the rollout array; VectorModel.fit and the training cycle forward to the WorldModel.
